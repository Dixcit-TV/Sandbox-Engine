...
void SDBX::DX11::DX11Render::Init(const Window& window, Viewport* viewports, size_t viewportCount)
{
	...
	SDBX_W_ASSERT_MSG(SUCCEEDED(res), L"Could not create DX11 device !");
	SDBX_W_ASSERT_MSG(featureLevel >= D3D_FEATURE_LEVEL_11_0, L"Supported feature level is below D3D_FEATURE_LEVEL_11_0 !");

	IDXGIDevice* dxgiDevice;
	res = m_pDxDevice->QueryInterface(__uuidof(IDXGIDevice), reinterpret_cast<void**>(&dxgiDevice));
	SDBX_W_ASSERT_MSG(SUCCEEDED(res), L"Could not query DXGI device !");

	IDXGIAdapter* dxgiAdapter;
	res = dxgiDevice->GetAdapter(&dxgiAdapter);
	SDBX_W_ASSERT_MSG(SUCCEEDED(res), L"Could not query DXGI adapter !");

	IDXGIFactory* dxgiFactory;
	res = dxgiAdapter->GetParent(__uuidof(IDXGIFactory), reinterpret_cast<void**>(&dxgiFactory));
	SDBX_W_ASSERT_MSG(SUCCEEDED(res), L"Could not query DXGI factory !");

	IDXGIOutput* tempOutput{};
	res = dxgiAdapter->EnumOutputs(0, &tempOutput);
	SDBX_W_ASSERT_MSG(SUCCEEDED(res), L"Failed to get temp IDXGIOutput from adapter !");

	UINT numOutput{ };
	res = tempOutput->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, NULL, &numOutput, nullptr);
	SDBX_W_ASSERT_MSG(SUCCEEDED(res), L"Failed to request the number of Adapter outputs !");
	...
	SDBX_W_ASSERT_MSG(SUCCEEDED(res), L"Failed to request Adapter output list !");
	...
	SDBX_W_ASSERT_MSG(SUCCEEDED(res), L"Failed to create swap chain !");

	ID3D11Texture2D* pbackBuffer;
	res = m_pDxSwapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(&pbackBuffer));
	SDBX_W_ASSERT_MSG(SUCCEEDED(res), L"Failed to query swap chain's back buffer !");
	...
	SDBX_W_ASSERT_MSG(SUCCEEDED(res), L"Failed to create render target UAV !");
	...
	SDBX_W_ASSERT_MSG(SUCCEEDED(res), L"Failed to create depth buffer texture !");
	...
	SDBX_W_ASSERT_MSG(SUCCEEDED(res), L"Failed to create depth buffer UAV !");
